...
function HotwireWindow:wireConnected(first, second)
    if first == wires[self.firstWire] and second == wires[self.secondWire]
        or second == wires[self.firstWire] and first == wires[self.secondWire] then													-- true, false
            sendClientCommand(self.character, 'BetterLockpickingContinuedVehicle', 'cheatHotwire', { vehicle = self.character:getVehicle():getId(), hotwired = true, broken = false })
			self:close()
    end
end
...
function HotwireWindow:initialise()
    ...
    BetterLockpickingContinued.Wires.addWires(self)
end
